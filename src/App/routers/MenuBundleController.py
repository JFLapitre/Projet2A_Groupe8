...
def update_predefined_bundle(
    ...
    item_ids: Optional[List[int]] = Query(None, description="Nouvelle liste des IDs d'items dans la composition."),
    service=Depends(get_service),
):
    try:
...
def update_discounted_bundle(
    ...
    required_item_types: Optional[List[str]] = Query(
        None, description="Nouvelle liste des types d'items auxquels la réduction s'applique (ex: 'sushi', 'boisson')."
    ),
    service=Depends(get_service),
):
    try:
