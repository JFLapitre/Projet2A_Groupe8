...
def test_delivery_constructor_ok():
    ...
    delivery = Delivery(
        id_delivery=1, driver=driver, orders=[order], status="in_progress", delivery_time=date(2025, 1, 2)
    )
...
def test_delivery_invalid_driver_type_raises_validationerror():
    ...
    with pytest.raises(ValidationError):
        Delivery(
            id_delivery=1, driver="invalid_driver", orders=[order], status="in_progress", delivery_time=date(2025, 1, 2)
        )
